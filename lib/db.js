...
export function insertCamera(db, cam) {
  const stmt = db.prepare(`
    INSERT INTO cameras (name, host, port, username, password, rtsp_url, snapshot_url, enabled, sort_order, poll_ms)
    VALUES (@name, @host, @port, @username, @password, @rtsp_url, @snapshot_url, @enabled, @sort_order, @poll_ms)
  `);
