...
import { rtspGrab, isAvailable as rtspGrabAvailable } from './snapshot-methods/rtsp-grab.js';
...
// Per-authClass backoff. Reolink keeps separate session pools for the HTTP API
// and RTSP, so a "max session" rejection on http-api should NOT stop us from
// trying rtsp-grab — only sibling http-api methods.
const hostBackoff = new Map(); // `${host}:${authClass}` -> { until, reason }
...
async function runLadder(camera, res) {
  ...
  for (const method of ladder) {
    // Skip methods whose auth-class is currently in backoff (e.g. http-api
    // saturated by other clients). Other auth-classes (rtsp) still run.
    const backoff = hostBackoff.get(backoffKey(camera.host, method.authClass));
    if (backoff && backoff.until > Date.now()) {
      const secs = Math.ceil((backoff.until - Date.now()) / 1000);
      attempts.push({ method: method.id, error: `backing off (${secs}s): ${backoff.reason}` });
      continue;
    }
    try {
      await method.run(camera, res);
      lastWorking.set(camera.id, { method: method.id, failCount: 0 });
      hostBackoff.delete(backoffKey(camera.host, method.authClass));
      attempts._served = true;
      return attempts;
    } catch (err) {
      attempts.push({ method: method.id, error: err.message, authClass: method.authClass });
      if (method.id === 'reolink-token') reolink.invalidateToken(camera.host);
      if (isRateLimitError({ error: err.message })) {
        hostBackoff.set(backoffKey(camera.host, method.authClass), {
...
function backoffKey(host, authClass) {
  return `${host}::${authClass}`;
}
...
function methodsFor(vendor, camera) {
  ...
  if (vendor === 'reolink' && camera.username) {
    out.push({ id: 'reolink-token', authClass: 'http-api', run: runReolinkToken });
  }
  if (camera.snapshot_url) {
    out.push({ id: 'url-creds', authClass: 'http-api', run: runUrlCreds });
  }
  // RTSP uses a separate session pool from HTTP, so it works as a fallback
  // when the camera's HTTP API is full (other clients holding sessions, or
  // a temporary account lockout).
  if (camera.rtsp_url && rtspGrabAvailable()) {
    out.push({ id: 'rtsp-grab', authClass: 'rtsp', run: rtspGrab });
  }
  return out;
}
...
export function forgetCamera(id, host) {
  ...
  if (host) {
    ...
    for (const cls of ['http-api', 'rtsp']) hostBackoff.delete(backoffKey(host, cls));
  }
}
