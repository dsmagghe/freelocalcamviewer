...
// When a camera rejects our login (rate-limit, max-sessions, bad creds), retrying
// at every poll just makes it worse — Reolink keeps the session-table full and
// honest sessions can't get in. Park the host for a while.
const loginFailures = new Map(); // host -> { until, reason }
const FAILURE_BACKOFFS = {
  max_session: 30_000, // server is full, give other sessions time to time out
  default:     10_000, // generic login error
};
...
export async function getToken(camera) {
  ...
  // If a recent login was rejected, don't hammer the camera — return the
  // cached error until the backoff window expires.
  const fail = loginFailures.get(camera.host);
  if (fail && fail.until > Date.now()) {
    const secs = Math.ceil((fail.until - Date.now()) / 1000);
    throw new Error(`backing off (${secs}s): ${fail.reason}`);
  }
  ...
  const promise = login(camera).then(
    (fresh) => {
      ...
      loginFailures.delete(camera.host);
      return fresh.token;
    },
    (err) => {
      tokenCache.delete(camera.host);
      const reason = err.message;
      const kind = /max session|max user|rspCode"?\s*:\s*-?5/i.test(reason) ? 'max_session' : 'default';
      loginFailures.set(camera.host, {
        until: Date.now() + FAILURE_BACKOFFS[kind],
        reason,
      });
      throw err;
    }
...
export function invalidateToken(host) {
  ...
  // Don't clear loginFailures here — invalidation can come from a snapshot
  // 401 *while* we already know logins are being rate-limited. Let the
  // backoff timer expire naturally.
}
